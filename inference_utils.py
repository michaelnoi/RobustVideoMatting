...
class VideoWriter:
    def __init__(self, path, frame_rate, bit_rate=1000000):
        ...
        # Convert frame_rate to a proper fraction for av library
        from fractions import Fraction
        if hasattr(frame_rate, 'numerator'):  # Already a fraction
            frame_rate_frac = frame_rate
        else:
            # Convert to fraction to avoid precision issues
            frame_rate_float = float(frame_rate)
            frame_rate_frac = Fraction(int(frame_rate_float * 1000), 1000)  # 3 decimal precision
        self.stream = self.container.add_stream('h264', rate=frame_rate_frac)
